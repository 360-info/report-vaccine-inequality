...
# locate total row (so we can chuck it and the footnotes)
india <- get_india_exports()
total_row_i <- str_which(india[[1]], "Total")

india %>%
  head(-3) %>%
  tail(-(nrow(india) - total_row_i + 1)) %>%
...
india %>%
  ...
  write_csv(here("out", "india-exports-totals.csv")) %>%
  {
    ggplot(.) +
      ...
      coord_flip() +
      labs(
        x = NULL, y = toupper("Number of vaccine doses exported"),
